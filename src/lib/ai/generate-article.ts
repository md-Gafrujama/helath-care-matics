...
const ArticleSchema = z.object({
  ...
  meta_title: z
    .string()
    .describe(
      "Browser/SEO <title>. Include focus keyword near the start. End with ' | HealthMatics'. MAXIMUM 60 characters including the brand suffix. Shorter is fine.",
    ),
  meta_description: z
    .string()
    .describe(
      "SEO meta description AND Open Graph description. MAXIMUM 160 characters. Shorter is fine. Include focus keyword once, a concrete hook, and a reason for healthcare leaders to click. No quotes.",
    ),
  og_title: z
    .string()
    .describe(
  ...
  geo_summary: z
    .string()
    .describe(
      "Generative-engine summary: 2-3 neutral sentences a model can cite. Include the key fact and who it affects (healthcare leaders). MAXIMUM 400 characters. Shorter is fine.",
    ),
...
/**
 * Step 1: Web-search trending healthcare coverage for ONE navbar topic,
...
export async function researchTopic(
  ...
  const avoidList =
    recentTitles.length > 0
      ? `Do NOT repeat or closely rehash any of these already-published HealthMatics titles:\n${recentTitles.map((t) => `- ${t}`).join("\n")}`
      : "No prior HealthMatics articles yet.";
  ...
  const messages: Anthropic.MessageParam[] = [
    {
      role: "user",
      content: `You are the research desk for HealthMatics, an editorial site for healthcare leaders and healthcare decision-makers.
...
YOUR JOB:
1. Use web search to scan DIFFERENT reputable sites for what is trending / newly reported RIGHT NOW in "${topic.name}" for healthcare leaders, CIOs, CMOs, and healthcare decision-makers.
2. Search across multiple outlets - e.g. HIMSS, Healthcare Dive, Fierce Healthcare, Becker's, Modern Healthcare, STAT News, Reuters Health, Kaiser Health News, Gartner, McKinsey Health, and major health policy sources - not just one domain.
3. Prefer angles that appeared in the last 7-14 days (or are clearly escalating now).
4. Pick ONE concrete, newsworthy story angle that HealthMatics can cover uniquely for healthcare leaders.
5. The angle MUST clearly belong under "${topic.name}" - not a different navbar topic.

Suggested search patterns (run several):
- "${topic.name} healthcare news"
- "trending ${topic.name} healthcare"
- "${topic.name} healthcare leaders 2026" (or current year)
...
HEALTHCARE IMPLICATION: why healthcare leaders care
...
export async function researchTitle(
  ...
  const avoidList =
    recentTitles.length > 0
      ? `Do NOT copy or closely rehash any of these already-published HealthMatics titles:\n${recentTitles.map((t) => `- ${t}`).join("\n")}`
      : "No prior HealthMatics articles yet.";
...
2. Search HIMSS, Healthcare Dive, Workforce, HBR, Bloomberg Law, Reuters, WSJ, HHS, ONC, CMS, FDA, Mercer, Gartner, and health policy sources.
...
Build slug, meta_title, og_title, focus_keyword, and the body around it.`
  ...
  const response = await client.messages.parse({
    ...
    messages: [
      {
        role: "user",
        content: `Using the research brief below, write a full article for HealthMatics (publication for healthcare leaders and healthcare decision-makers). Tone: clear, editorial, non-hype - like Healthcare Dive or healthcare executive networks. Concrete and specific.
...
SEO RULES (required):
${titleRule}
2. meta_title MAXIMUM 60 characters (including " | HealthMatics"). Shorter is fine.
3. meta_description MAXIMUM 160 characters. Shorter is fine. Hook + keyword + healthcare leader benefit (also used as OG description).
