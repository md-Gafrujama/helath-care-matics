...
export default function SubscribeModal() {
  ...
  const [description, setDescription] = useState(
    "Tell us where to send it. You will also get The Brief, our weekday newsletter for healthcare leaders.",
  );
  ...
  useEffect(() => {
    const onOpen = (e: Event) => {
      ...
      if (detail?.title) {
        ...
        setDescription(
          "The weekday newsletter healthcare leaders read before the first meeting. Free.",
        );
      }
      setOpen(true);
    };

    window.addEventListener("healthmatics:open-modal", onOpen);
    return () => window.removeEventListener("healthmatics:open-modal", onOpen);
  }, []);
  ...
  const submit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    window.dispatchEvent(
      new CustomEvent("healthmatics:toast", {
...
export function Toast() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("healthmatics:toast", handler);
    return () => window.removeEventListener("healthmatics:toast", handler);
  }, []);
